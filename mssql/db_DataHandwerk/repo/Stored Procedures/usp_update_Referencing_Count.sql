...
UPDATE repo.RepoObject
  SET
      [RepoObject_Referencing_Count] = [rorc].[Referencing_Count]
...
(
    ...
    FROM
         [repo_sys].[RepoObjectReferencing] AS [ror]
    GROUP BY
             [RepoObject_guid]
) AS [rorc]
       ON [repo].[RepoObject].[RepoObject_guid] = [rorc].[RepoObject_guid]
WHERE
      ISNULL([repo].[RepoObject].[RepoObject_Referencing_Count] , 0) <> ISNULL([rorc].[Referencing_Count] , 0)
...
WHERE
      ISNULL([repo].[RepoObjectColumn].[Referencing_Count] , 0) <> ISNULL([rorc].[Referencing_Count] , 0)
...
EXEC repo.usp_execution_log__insert
   ...
   , @info_09 = NULL
GO
EXECUTE sp_addextendedproperty @name = N'RepoObject_guid', @value = N'50e4634c-304c-eb11-84d3-a81e8446d5b0', @level0type = N'SCHEMA', @level0name = N'repo', @level1type = N'PROCEDURE', @level1name = N'usp_update_Referencing_Count';
