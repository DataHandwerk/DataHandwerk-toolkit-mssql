...
AS
	...
	INSERT INTO repo.RepoObjectSource_FirstResultSet ([RepoObject_guid]
	, [column_ordinal]
	, [target_column_name]
	, [system_type_id]
	, [system_type_name]
	...
	, [created_dt])
		...
		FROM (SELECT
			...
			LEFT JOIN (SELECT
				...
				GROUP BY [RepoObject_guid]) AS ros
			...
			AND ([ros].[created_dt_min] IS NULL
			OR [ro].[SysObject_modify_date] IS NULL
			OR [ros].[created_dt_min] < [ro].[SysObject_modify_date])) AS ro_filtered
		CROSS APPLY
