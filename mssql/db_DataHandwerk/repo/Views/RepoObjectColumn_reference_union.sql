...
SELECT
	 ...
	 , [is_referenced_object]
FROM
     [repo].[RepoObjectColumn_reference_QueryPlan] AS roc_r
WHERE
--only views
[referencing_type] = 'V'
...
(
    SELECT
           1
    FROM
         [repo].[RepoObjectColumn_reference_FirstResultSet] AS [roc_r_common]
    WHERE  [is_referenced_object] = 1
...
(
    SELECT
           1
    FROM
         [repo].[RepoObjectColumn_reference_BySamePredecessors] AS [roc_r_derived]
